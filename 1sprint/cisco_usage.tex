...
With this code it is possible to create an intermediate service for the Cisco MSE RESTful api, that is able to intercept a response and obfuscate certain information. To use this service, all that is required is knowledge of the IP address and login information. To automate the requests and create a flow of data from the MSE systems to the aSTEP database, we will have to build and implement a client that fetches data at an interval. The scope of the project is beyond AAU, and as such we require the client to be capable of handling more then a single MSE system. A model showing how the systems interact can be seen on \cref{fig:system_flow}.
