...
Triangulation works by having three accesspoints being able to connect to the device. The position of the accesspoints are known to the system and by calculating the distance from the device to each accesspoint, based on the signal strength, the position can be found. It is also possible to find the direction of the device, relative to the accesspoints and from this information calculate the position. The two methods can also be combined such that the system is based on signal strength and direction\cite{Triangulation}.
...
%Next subsub?
%Something about fingerprinting
As mentioned it is also possible to use fingerprinting for indoor positioning.
